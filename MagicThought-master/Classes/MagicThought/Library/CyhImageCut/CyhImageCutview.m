...
- (instancetype)setView_cutViewWithImage:(UIImage *)Oimage addSuperclassView:(UIView *)Spview PinScale:(CGFloat)PinScale complet:(void (^)(UIImage *))resultComplet
{
    ...
    self.Cutview.frame = CGRectMake(0, 0, self.frame.size.width,
                                    self.whScale > 0 ? self.frame.size.width * self.whScale : self.frame.size.width);
...
- (void)sureCutImage
{
    ...
    [CyhImageCutTool AddImage:self.Oimage setImageCutFrame:CGRectMake(self.cutX*self.cutImageScale, self.cutY*self.cutImageScale, cutWH,self.whScale > 0 ? cutWH * self.whScale : cutWH) comple:^(UIImage *finishImage) {
